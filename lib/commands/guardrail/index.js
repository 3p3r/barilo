  ...
  while (more) {
    ...
    switch (type) {
      ...
      case ACTIONS.SQS:
        ...
        const arn = await getQueueArnByName(queue);
        step = new tasks.SqsSendMessage(stack, `SQS${index}`, {
          queue: sqs.Queue.fromQueueArn(stack, `Queue${index}`, arn),
      ...
      case ACTIONS.SNS:
        ...
        const arn = await getTopicArnByName(topic);
        step = new tasks.SnsPublish(stack, `SNS${index}`, {
          topic: sns.Topic.fromTopicArn(arn),
